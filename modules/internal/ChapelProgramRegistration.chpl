...
module ChapelProgramRegistration {
  ...
  record chpl_programInfo {
    ...
    inline proc ref setCallback(param name: string) {
      ...
      // TODO: This absolutely, should not, under ANY circumstances, create a
    ...
    inline proc ref markDataAsPrepared() {
      param cname = 'chpl_program_info_set_is_data_prepared';
      extern cname proc set(ref info: chpl_program_info, x: c_int): c_int;
      return set(info, 1);
    }
  ...
  // TODO --> If we can use arbitrary expressions for linkage on types
  //          and other symbols then we definitely can. But we'll have
  //          to expose the helper functions here for that module to call.
  ...
  // Each entry here mirrors an entry in 'chapel-program-data-macro.h'.
