...
#ifndef CHPL_LIB_PARSING_BISON_BISON_PARSER_H
#define CHPL_LIB_PARSING_BISON_BISON_PARSER_H
...
#ifdef PARSER_DEBUG
  #define BISON_PARSER_DEBUG PARSER_DEBUG
#else
  #define BISON_PARSER_DEBUG 0
#endif
...
void BisonParser::updateParseResult(ParserContext* parserContext) {
...
BuilderResult
BisonParser::parseFile(const char* path, ParserStats* parseStats) {
  owned<Builder> builder;
  if (parentSymbolPath_.isEmpty()) {
    builder = Builder::createForTopLevelModule(context_, path);
  } else {
    builder = Builder::createForIncludedModule(context_, path,
                                               parentSymbolPath_);
  }
  ...
  if (BISON_PARSER_DEBUG) yychpl_debug = 1;
...
BuilderResult
BisonParser::parseString(const char* path, const char* str,
                         ParserStats* parseStats) {
  owned<Builder> builder;
  if (parentSymbolPath_.isEmpty()) {
    builder = Builder::createForTopLevelModule(this->context_, path);
  } else {
    builder = Builder::createForIncludedModule(this->context_, path,
                                               parentSymbolPath_);
  }
