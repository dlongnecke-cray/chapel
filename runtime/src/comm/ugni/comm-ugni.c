...
void rf_handler(gni_cq_entry_t* ev)
{
  ...
  case fork_op_small_call:
             ...
             (int) req_li, sprintf_rf_req(-1, f));
    {
      ...
      if (f_c->comm.fast) {
        chpl_rt_callFtableEntryHere(CHPL_PROGRAM_ROOT, f_c->comm.fid, f);
...
{
  // Call the on body
  chpl_rt_callFtableEntryHere(CHPL_PROGRAM_ROOT, f->comm.fid, f);
...
void fork_call_wrapper_large(fork_large_call_info_t* lc)
{
  ...
  *bundle = lc->hdr;
  ...
  // Call the on body
  chpl_rt_callFtableEntryHere(CHPL_PROGRAM_ROOT, bundle->comm.fid, bundle);
